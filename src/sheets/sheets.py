...
class Sheets(Tool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not os.path.exists(self.credentials_path):
            raise ValueError(
                f"The config file {os.getenv('GOOGLE_SHEETS_CONFIG_FILE')} does not exist"
            )
    ...
    def check_arguments(self, arguments: argparse.Namespace) -> None:
        ...
        if extension not in SUPPORTED_EXTENSIONS:
            raise ValueError(
                f"The data file {arguments.data_file!r} is a {extension!r} file."
    ...
    def run(self, arguments: argparse.Namespace) -> None:
        ...
        print_success(
            f"Successfully appended the data to the spreadsheet {self.spreadsheet_id!r}"
        )
