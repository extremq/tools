...
class MoneyLost(Tool):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.from_currency = {"code": None, "rate": None}

        self.to_currency = {"code": None, "rate": None}
    ...
    def check_arguments(self, arguments: argparse.Namespace) -> None:
        if arguments.from_currency.upper() == arguments.to_currency.upper():
            raise ValueError(
                f"From currency ({arguments.from_currency.upper()!r}) "
                f"and To currency ({arguments.to_currency.upper()!r}) cannot be the same."
            )

        if arguments.sold_amount <= 0:
            raise ValueError(
                f"Sold amount ({arguments.sold_amount!r}) must be greater than 0."
            )

        if arguments.bought_amount <= 0:
            raise ValueError(
                f"Bought amount ({arguments.bought_amount!r}) must be greater than 0."
            )

        if arguments.from_currency.upper() not in self.exchange_rates["rates"]:
            raise ValueError(
                f"From currency ({arguments.from_currency.upper()!r}) is not supported."
            )

        if arguments.to_currency.upper() not in self.exchange_rates["rates"]:
            raise ValueError(
                f"To currency ({arguments.to_currency.upper()!r}) is not supported."
            )
    ...
    def run(self, arguments: argparse.Namespace) -> None:
        ...
        print(
            f" Your rate: {user_exchange_rate:10.3f} {self.from_currency['code']} / per {self.to_currency['code']}"
        )
        print(
            f" Real rate: {real_exchange_rate:10.3f} {self.from_currency['code']} / per {self.to_currency['code']}"
        )
        print()
