    ...
    def wrapper(*args, **kwargs):
        for i in range(8):
            try:
                ...
                print_error(
                    f"Got rate limited, retrying {func.__name__} in {2 ** i} seconds..."
                )
                time.sleep(2**i)
            except Exception:
                raise
        raise Exception(
            "Tried 10 times, cannot get data. Check if the API key is valid or if the server is down."
        )
...
def get_puuid(summoner_name: str, region: str):
    ...
    headers = {"X-Riot-Token": os.getenv("LEAGUE_API_KEY")}
    ...
    raise Exception(
        f"Cannot get puuid, check if the summoner "
...
def get_match_history(puuid: str, start_time: str, end_time: str):
    ...
    while True:
        ...
        def _get_match_history():
            base_url = (
                f"https://europe.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?start={start}"
            ...
            raise Exception(
                f"Cannot get match history, check if the summoner "
                f"name is correct or if the API key is valid. Error: {response.json()}"
            )
...
def filter_match_data(match_data: dir, puuid: str, full_data: bool):
    if full_data:
        ...
        simple_data = {
            ...
            "start_time": timestamp_milis_to_datetime(
                match_data["info"]["gameStartTimestamp"]
            ),
            "stop_time": timestamp_milis_to_datetime(
                match_data["info"]["gameEndTimestamp"]
            ),
        ...
        for player in match_data["info"]["participants"]:
            if player["puuid"] == puuid:
                ...
                simple_data["kda"] = (player["kills"] + player["assists"]) / player[
                    "deaths"
                ]
...
def get_match_data(match_id: str):
    ...
    raise Exception(
        f"Cannot get match data, check rate limits or if the API key is valid. Error: {response.json()}"
    )
...
def turn_data_to_dataframe(match_ids: list[dir], puuid: str, full_data: bool):
    ...
    for match_id in match_ids:
        ...
        pd.DataFrame([filtered_match_data]).to_csv(
            f"match_data.csv",
            mode="a",
