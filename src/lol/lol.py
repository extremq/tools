...
class LeagueOfLegends(Tool):
    ...
    def run(self, arguments: argparse.Namespace) -> None:
        ...
        self.match_history = get_match_history(
            self.puuid, self.start_date, self.end_date
        )
        print_info(f"Finished getting {len(self.match_history)!r} matches.")

        self.match_data = turn_data_to_dataframe(
            self.match_history, self.puuid, self.full_data
        )
        ...
        print_success(
            f"Successfully saved {len(self.match_data)!r} matches to {arguments.output!r}."
        )
